  ...
  >
    <n-form ref="profileFormRef" :model="profileForm" :rules="profileRules" label-placement="top">
      ...
      <div v-if="profileForm.target_type === 'channel'" class="config-form__section">
        ...
        <n-form-item :label="t('config.mentionInitiatorInChannel')" style="margin-bottom: 4px">
...
.config-notification-profile__top,
.config-notification-profile__footer {
  display: flex;
  justify-content: space-between;
  gap: 12px;
...
.config-notification-profile__title {
  font-size: 16px;
  font-weight: 600;
}

.config-notification-profile__meta {
  margin-top: 4px;
  font-size: 13px;
  color: rgba(15, 23, 42, 0.56);
}

.config-notification-profile__section {
  display: grid;
  gap: 8px;
}

.config-notification-profile__label {
  font-size: 12px;
  color: rgba(15, 23, 42, 0.62);
  text-transform: uppercase;
  letter-spacing: 0.02em;
