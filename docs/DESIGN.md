...
| 步骤完成 | "  ✓ 步骤 N: {描述}" |
| 任务成功完成 | "✅ MR 已创建: !{iid}" |
| 任务执行失败 | "❌ 任务失败: {error_message}" |

- **开始反馈**：当 Worker 开始执行任务时发送，给用户及时确认
- **规划反馈**：规划完成后更新 MR 描述，让用户了解实现方案
- **步骤反馈**：每个步骤完成后更新 MR 进度
- **完成反馈**：任务完成后发送，包含 MR 链接或错误信息
- **status 命令**：用户可随时通过 `@ai-bot status` 查询当前任务状态
...
| 阶段 | 内容 | 交付物 |
|---|---|---|
| **P0 - 最小可用** | Webhook 接收 → 解析 @bot (幂等) → Docker 容器内执行 (agentic Claude CLI) → 创建分支/MR → 回复 issue (开始/完成反馈) | 能跑通完整流程的 MVP |
| **P0.1 - 分步规划** | 规划 → 更新MR → 逐步执行 → 实时更新MR进度 → 完成报告 | 增强用户体验和可追踪性 |
...
目标：增强用户体验，提供可追踪的实现过程。

- **规划阶段**：调用 Claude 分析需求，生成实现步骤清单
- **MR 描述更新**：将规划文档写入 MR 描述，让用户提前了解实现方案
- **逐步执行**：按步骤顺序执行，每个步骤调用一次 Claude
- **进度追踪**：每个步骤完成后更新 MR 描述，标记进度和执行日志
- **完成报告**：所有步骤完成后生成最终报告，包含变更文件列表和执行摘要
- **工具启用**：启用 Claude Tool Use，支持 Bash/Read/Write 工具执行
