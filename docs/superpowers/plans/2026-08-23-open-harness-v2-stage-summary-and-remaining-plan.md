...
| L5 发布验收 | **未完成** | 已补充 390×844 创建/详情、长文本、编辑器焦点、底部操作区、创建表单与已有 Issue 的四 Harness 选择、真实运行态 command/ACK/刷新连续性与模式显示修复；structured SSE 已增加 stale-source 生命周期防护并通过前端全量回归与 production build；见 [R4.3/R4.4 live Host evidence](../evidence/2026-09-04-open-harness-v2-r4.3-r4.4-live-host.md) | 移动键盘/安全区、有效的真实断线重连、`v2_only` V1 只读展示、完整交互/运维/安全阻断清单与签署 |
...
| R4.3 | **部分 evidence，未签署**：已覆盖 390×844 与桌面真实交互、长文本、编辑器焦点、底部操作区、创建表单与已有 Issue 的四 Harness 选择、实时 command/ACK、刷新连续性、运行完成过渡与 `freeform` 模式显示；另做了两次 remote backend-only restart probe（Tasks 369/370），但均因上游 `rate_limited` 在延迟命令前终止，未形成有效断线重连证据；本轮补充 structured SSE stale-source 生命周期防护及 3 个竞态回归测试，前端全量回归 80 files/1691 tests、production build 通过；同时补跑 `v2_only` V1 只读源码边界（后端 9 tests、TaskView 4 tests 通过），但未切换 Host 模式；见 live Host evidence | 仍需移动键盘/安全区、有效的真实断线重连、Host 上 `v2_only` 下的 V1 只读展示，并由验收人确认无阻断交互缺陷 |
