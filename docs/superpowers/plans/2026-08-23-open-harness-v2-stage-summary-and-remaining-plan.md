...
| R4.4 | **部分 evidence，未签署**：已覆盖现有真实 Task（Tasks 357–366、368–378；四 Harness 均有真实成功样本，但 generation-73 当前 Bundle 仅 Pi/Claude/OpenCode 成功，Codex Tasks 377/378 是正确分类的 Provider `rate_limited`/region-blocked `engine_error` 负向样本；Task 373 的 Pi session mismatch 已作为 superseded defect 排除，Task 374/375/376 在 Bundle 166/167/168 成功，Task 368 保留为 preceding-generation Codex success，Task 371 完成 nginx-only 重连，Task 372 完成稳定态取消，369/370 保留为上游限流负向样本）、startup/失败分类、command latency、usage、canonical terminal、archive、raw-log、delivery、seq 连续性，以及当前活跃队列/Issue lock/secret-like 扫描快照；Bundle 166–169 的 5 个 attempt/111 条 receipt 均为唯一 terminal、连续 seq；Task 372 的取消语义按冻结合同以 `run.failed(status=cancelled, failure.kind=cancelled, exit_code=143)` 收敛，不能按 terminal event type 单独判为 mismatch；当前 cohort 复核确认状态/terminal 语义一致、无重复 terminal/seq gap；2026-09-04T15:03:49Z 只读复核确认 374–376 仍成功、377/378 仍失败且无新的 Codex-legal Provider，因此未重复创建已知受限 Provider 任务；本地 Mattermost mock E2E 96 项通过，且目标 Host 内网 mock Mattermost 探针实际产生 1 次 HTTP post/1 条 success delivery 后已清理；本探针仅证明 Host backend/DB/HTTP 链路，不等于真实 Mattermost 或权限审计 | 仍需在 exact generation-73 candidate 上取得 Codex success 或由发布评审明确接受 Provider 可用性边界，并按 Harness/Profile/Host 完整审阅告警行为与阻断指标，正式确认错误成功、串线、凭据泄漏、无法取消、重复 terminal/seq 缺口均为 0；开发 Host 当前未配置通知 profile，真实 Mattermost 告警投递尚未实测；全库更早历史仍有 12 个未 terminal attempt，未回填且不计入当前 candidate 通过，见 evidence |
