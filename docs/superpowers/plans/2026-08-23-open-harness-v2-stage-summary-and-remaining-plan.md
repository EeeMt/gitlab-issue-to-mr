...
| R4.3 | **部分 evidence，未签署**：已覆盖 390×844 与桌面真实交互、长文本、编辑器焦点、底部操作区、创建表单与已有 Issue 的四 Harness 选择、实时 command/ACK、刷新连续性、运行完成过渡与 `freeform` 模式显示；另做了两次 remote backend-only restart probe（Tasks 369/370），但均因上游 `rate_limited` 在延迟命令前终止，未形成有效断线重连证据；见 live Host evidence | 仍需移动键盘/安全区、有效的真实断线重连、`v2_only` 下的 V1 只读展示，并由验收人确认无阻断交互缺陷 |
