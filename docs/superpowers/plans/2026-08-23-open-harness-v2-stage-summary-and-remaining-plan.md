...
candidate 的 Kit/CLI 启动边界也已实现并完成 Host evidence，且本轮 Codex Adapter 模型投影修复已生成
...
| L2 源码/测试 | **当前 candidate 通过** | Kit provenance、Snapshot CLI identity、Scheduler/Worker/launcher 热路径边界与聚焦回归已证明；全量单元测试有 3247 passed 基线；`8110afa0` 的 Codex `OPENAI_MODEL` 投影修复及受影响 Bundle/Profile/Scheduler/notification/freeform 回归 227 passed、lint/secret scan 通过；structured SSE source-identity 防护与本轮移动安全区修复后前端全量回归 80 files/1692 tests、production build 通过 | 若 R4.3–R4.5 发现源码变化，按影响面重开 |
| L3 不可变 composition | **当前 candidate 通过** | 新 Kit 已完整安装，Profile 4 generation 72 管理员 Verify 四 Harness；Bundle 163（Codex-selected）与 OpenCode-specific Bundle 164 及 Image/Kit/Profile/Adapter identity 可追溯 | R4 签署前保持 identity 不漂移 |
| L4 真实 Host/Task | **R4.1 scope 通过** | 新 Kit、四 Harness admin/launcher smoke、5 条 warm-start 成功 Task、TTL 过期后的成功路径、受控 selected-CLI 失败、修复后 Codex Task 368，以及真实 OpenCode Task 371 均有证据 | 各 Harness 的正式 L5 交互/运维审阅与签署 |
...
| R4.2 | **当前 candidate 已冻结（待 R4 签署）**：完成 R3 后源码/组合影响面审计，生成新 Kit/Bundle，并在目标 Host 完整 Verify | Image/Kit/Profile/Bundle/Adapter/Host identity、Profile generation 72、Bundle 163（Codex-selected）与 OpenCode-specific Bundle 164、管理员四 Harness Verify、修复后 Task 368 与 Task 371 evidence 已记录；163/164 仅因所选 Harness 验证证据形成 scoped Bundle variant，签署前不得发生运行时身份漂移 |
...
| R4.4 | **部分 evidence，未签署**：已覆盖十五条当前真实 Task（Tasks 357–366、368–372；四 Harness 各有真实成功样本，并保留 Claude/Codex 多 Provider `rate_limited`/`engine_error` 失败分类；修复后 Task 368 使用 Bundle 163 且正确显示 `minimax/minimax-m3:free`，Task 371 使用 OpenCode-specific Bundle 164 完成真实 nginx-only 重连 spot-check，Task 372 使用同一 Bundle 完成稳定态 OpenCode 取消，369/370 仍保留为上游限流负向样本）、startup/失败分类、command latency、usage、canonical terminal、archive、raw-log、delivery、seq 连续性，以及当前活跃队列/Issue lock/secret-like 扫描快照；Bundle 163 的 3 个 attempt/29 条 receipt 与 Bundle 164 的 2 个 attempt/41 条 receipt 均为唯一 terminal、连续 seq；Task 372 的取消语义按冻结合同以 `run.failed(status=cancelled, failure.kind=cancelled, exit_code=143)` 收敛，不能按 terminal event type 单独判为 mismatch；十五条 live cohort 复核另确认状态/terminal 语义一致、无重复 terminal/seq gap，canonical/raw secret-like 命中为 0；本地 Mattermost mock E2E 96 项通过，且目标 Host 内网 mock Mattermost 探针实际产生 1 次 HTTP post/1 条 success delivery 后已清理；本探针仅证明 Host backend/DB/HTTP 链路，不等于真实 Mattermost 或权限审计 | 仍需按 Harness/Profile/Host 完整审阅告警行为与阻断指标，并正式确认错误成功、串线、凭据泄漏、无法取消、重复 terminal/seq 缺口均为 0；开发 Host 当前未配置通知 profile，真实 Mattermost 告警投递尚未实测；全库更早历史仍有 12 个未 terminal attempt，未回填且不计入当前 candidate 通过，见 evidence |
