...
repo_delivery_run_python() {
    # All delivery git work runs as the unprivileged workspace owner, matching
    # the rest of the entrypoint. Arguments are shell-escaped with printf %q so
    # empty values, spaces and punctuation survive the inner login shell.
    local helper_args
...
repo_pin_delivery_start() {
    # Fix S (local task-branch HEAD), R0 (confirmed remote work-branch HEAD at
    # preparation) and B0 (confirmed base HEAD) before any pre-script or
    # harness code can move the workspace. REPO_REMOTE_WORK_SHA itself is
    # mutated by successful pushes, so it must never double as the task start.
...
repo_delivery_collect() {
    # Collect local facts (this-task commits, inherited pending commits, net
    # diff) into the snapshot with push.status=not_attempted. No network.
...
repo_delivery_remote_tip() {
    # Prints the current remote work-branch tip (or nothing when the branch is
    # absent). Returns 0 on a successful observation, nonzero on network/auth
    # failure — an unconfirmed observation must not be treated as "no branch".
    local refs tip
    set +e
...
repo_delivery_publish() {
    ...
    # The pinned head must still be the branch head with no stray changes;
    # otherwise results and pushed content would drift apart.
    if ! codify_run_shell \
    ...
    if [ -z "${remote_tip}" ]; then
        ...
        # The branch has never existed: create it with "must not exist" as the
        # only allowed lease.
    ...
    # Fast-forward proven locally (R <= H and R0 <= R when R0 existed). Use the
    # observed tip as the exact lease; never a force without that proof.
    ...
    # One bounded recheck after a refused or uncertain push: prove the remote
    # already contains H (server may have completed the write despite a
    # non-zero response, or a concurrent writer may have landed it).
